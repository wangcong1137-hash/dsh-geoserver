...
import { createPublicationEvent, deliverPublicationWebhook, publishLayerFile, } from './publishing.js';
...
export function resolveBaseUrl(value) {
    ...
    if (baseUrl === '') {
        throw new Error('GeoServer base URL is not configured. Open Settings → Plugins → Plugin configuration → GeoServer, '
 ...
 */
export function resolvePublishWorkspace(requested, configured) {
    const workspace = (requested ?? configured).trim();
    if (workspace === '') {
        throw new Error('GeoServer publication workspace is not configured. Set Default publication workspace in the '
 ...
 */
export async function resolveRuntimeCredentials(config, store) {
...
export function apply(ctx, config) {
    ...
    const credentials = () => resolveRuntimeCredentials(current(), ctx.get('credentials'));
        ...
        webServer.register({
            ...
            handler: (req, res) => {
                void (async () => {
                    ...
                    const patch = {};
                    const stringFields = ['baseUrl', 'username', 'defaultWorkspace', 'webhookUrl', 'webhookTokenEnv'];
                    const numberFields = ['publishMaxBytes', 'webhookTimeoutMs'];
                    const supportedFields = new Set([...stringFields, ...numberFields, 'publishRoots']);
                    ...
                    const unsetFields = Array.isArray(unset)
                        ? unset.filter((key) => typeof key === 'string' && supportedFields.has(key))
                        : [];
                    if (Object.keys(patch).length > 0 || unsetFields.length > 0) {
                        try {
                            ...
                            if (unsetFields.length > 0) {
                                await sctx.settings.mutate(GEOSERVER_SETTINGS_NAMESPACE, unsetFields.map(path => ({ op: 'unset', path: [path] })));
                            }
                        }
                        catch (error) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            return await fetchMapImage(baseUrl, params, await credentials(), exec.signal, timeoutMs, cache, imageBase);
        },
    }));
    ctx.tools.register(defineTool({
        name: 'geoserver_publish',
        description: 'Publish one local GeoTIFF (.tif/.tiff) or one ZIP containing a single SHP dataset to '
        ...
        async execute(args, exec) {
            ...
            const result = await publishLayerFile(baseUrl, await credentials(), {
                kind: args.kind,
            ...
            }, { allowedRoots: resolved.publishRoots, maxBytes: resolved.publishMaxBytes }, exec.signal, timeoutMs);
            ...
            const notification = resolved.webhookTokenEnv !== undefined && webhookToken === undefined
                ...
                : await deliverPublicationWebhook(resolved.webhookUrl, webhookToken, event, exec.signal, resolved.webhookTimeoutMs);
