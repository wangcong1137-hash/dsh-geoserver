 ...
 */
export function buildGetMapUrl(baseUrl: string, params: GetMapParams): string {
  ...
  url.searchParams.set(
    'transparent',
    params.transparent === undefined ? 'FALSE' : params.transparent ? 'TRUE' : 'FALSE',
  )
  return url.toString()
}
 ...
 */
export function parseRestList(json: unknown, kind: 'workspace' | 'layer' | 'style'): string[] {
  ...
  if (collection === undefined || typeof collection !== 'object' || collection === null) return []
  const items = asList<{ name?: unknown }>((collection as Record<string, unknown>)[kind])
  return items
 ...
 */
export function parseRestLayerDetail(
  json: unknown,
): { title?: string; srs?: string; bbox?: readonly [number, number, number, number] } {
  const root = json as Record<string, unknown> | undefined
  const layer = root?.['layer']
  if (layer === undefined || typeof layer !== 'object' || layer === null) return {}
  ...
  const styleList = stylesField !== null && typeof stylesField === 'object' && !Array.isArray(stylesField)
    ? (stylesField as Record<string, unknown>)['style']
    : stylesField
  const styles = asList<{ name?: unknown }>(styleList)
  ...
  return {
    ...
    ...(srs !== undefined ? { srs } : {}),
    ...(bbox !== undefined ? { bbox } : {}),
    ...(styles.length > 0 ? { styles: styles.map(name => ({ name }) as WmsStyleInfo) } : {}),
  }
}

function parseBoundingBox(value: unknown): readonly [number, number, number, number] | undefined {
 ...
 */
export function parseCapabilitiesXml(xml: string): CapabilitiesResult {
  interface LayerCtx {
    name?: string
    title?: string
    styles: WmsStyleInfo[]
  ...
  const stack: LayerCtx[] = []
  let title: string | undefined

  type Event =
    | { kind: 'open'; index: number }
    | { kind: 'close'; index: number }
    | { kind: 'text'; tag: 'Name' | 'Title'; content: string; index: number }
    | { kind: 'style'; name: string; index: number }
  ...
  for (const event of events) {
    ...
    if (event.kind === 'style') {
      ...
      if (ctx !== undefined) ctx.styles.push({ name: event.name })
      continue
    }
    // text event
    const ctx = stack[stack.length - 1]
    if (event.tag === 'Name') {
      if (ctx !== undefined && ctx.name === undefined) ctx.name = event.content
      ...
      if (stack.length <= 1) {
        if (title === undefined && event.content.length > 0) title = event.content
